                    ...
                    <div class="convert">
                        <form method="post">
                            <div class="form-group">
                                ...
                                <select name="from_coin" id="from" onchange="this.form.submit()">
                                    <option value="btc" <?php echo ($_POST['from_coin'] ?? '') == 'btc' ? 'selected' : ''; ?>>BTC</option>
                                    <option value="eth" <?php echo ($_POST['from_coin'] ?? '') == 'eth' ? 'selected' : ''; ?>>ETH</option>
                                    <option value="sol" <?php echo ($_POST['from_coin'] ?? '') == 'sol' ? 'selected' : ''; ?>>SOL</option>
                                    <option value="bnb" <?php echo ($_POST['from_coin'] ?? '') == 'bnb' ? 'selected' : ''; ?>>BNB</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="to">To:</label>
                                <select name="to_coin" id="to">
                                    <?php
                                    $from_coin = $_POST['from_coin'] ?? 'btc'; // Default to BTC if no selection
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="amount" id="amount" placeholder="Enter amount" style="height: 30px !important;">
                            </div>
                            <button type="submit" name="convert" style="width:fit-content;text-align:center;">Convert</button>
                        </form>
                    </div>
                </div>
                <div class="l_half">
                    <div class="conversion_history">
                        <h2>Convrsion History</h2>
                        <table>
                            <?php
                            // Assuming $con is your database connection and $userId is the user's ID

                            $query = "SELECT * FROM conversion_history WHERE uid = $userId ORDER BY 'date' DESC limit 5";
                            $result = mysqli_query($con, $query);
